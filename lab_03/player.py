...
class ComputerPlayer(Player):
    ...
    def evaluate(self, board: Board, cell_type: Cell) -> int:
        """
        Method to evalute current state of the board
        """
    ...
    def evalute_heura(self, board: Board, cell_type: Cell) -> int:
        """
        Method to evaluate heuristic value of the heuristic function for :cell_type: player and :board:
        """
        result = 0
...
class HumanPlayer(Player):
    ...
    def make_move(self, board: Board) -> Move:
        """
        Method allowing human player to make a move based on his input
        """
        y = input(f"Select a row to put {cells_sign[self.cell_type]} [1,2..{board.size}]: ")
        x = input(f"Select a collumn to put {cells_sign[self.cell_type]} [1,2..{board.size}]: ")
        while not x.isdigit() or not y.isdigit() or not board.is_legal_move(int(x) - 1, int(y) - 1):
            print("Invalid input. Select again")
            y = input(f"Select a row [1,2..{board.size}]: ")
            x = input(f"Select a collumn [1,2..{board.size}]: ")
