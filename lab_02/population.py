...
class Population:
    ...
    def update_sigma(self, tau) -> None:
        sigma = self.centroid.sigma * np.exp(tau * self.rng.normal(0.0, 1.0))#.multivariate_normal(mean=np.array([0]), cov = np.identity(1)))
        for element in self.population:
            element.sigma = sigma
        
    def update_self_adapt_sigma(self, tau) -> None:
        for element in self.population:
            sigma = self.centroid.sigma * np.exp(tau * self.rng.normal(loc=0, scale=1.0))#multivariate_normal(mean=np.array([0]), cov = np.identity(1)))
            element.sigma = sigma
